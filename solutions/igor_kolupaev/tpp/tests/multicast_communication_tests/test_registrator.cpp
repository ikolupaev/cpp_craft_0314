...
#endif
{
	...
	master_test_suite.add(BOOST_TEST_CASE(&trade_processor_tests));
	master_test_suite.add(BOOST_TEST_CASE(&quote_processor_tests));

	master_test_suite.add(BOOST_TEST_CASE(&udp_listener_tests));

	master_test_suite.add( BOOST_TEST_CASE( &configurator_tests ) );
